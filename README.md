...
2. **Сверка деклараций с ген. полисом (cargo)** — сверка деклараций на перевозку груза с генеральным полисом и ДС к нему (`POST /api/reconcile`, статусы Совпадает/Не совпадает). Подробности бизнес-логики — в `BUSINESS.md`.

> **Руководство оператора** — структура папок, промпты через `.env`, перевозчики, разбор ошибок и таймаутов:
> [`docs/Руководство оператора — сверка грузовых деклараций.html`](docs/Руководство%20оператора%20—%20сверка%20грузовых%20деклараций.html) (открывается в браузере)
> Онлайн-версия: https://claude.ai/code/artifact/004df7b0-d79a-4519-b2d1-05aec715e9f7
> По вопросам работы ассистента — Матьякубов Александр, matyakubov@vsk.ru
